...
class Atom10_FeedEntryContributorTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $input = sprintf('<feed xmlns="http://www.w3.org/2005/Atom"><entry><contributor>%s</contributor></entry></feed>', $input);
        $feed = \ComplexPie\ComplexPie($input);
        list($person) = $feed->entries[0]->contributors;
        if ($expected !== null)
        {
...
EOF;
        ...
        $this->assertSame('Contributor 1', $feed->entries[0]->contributors[0]->name->to_text());
        $this->assertSame('Contributor 2', $feed->entries[0]->contributors[1]->name->to_text());
    }
...
EOF;
        $feed = \ComplexPie\ComplexPie($input);
        $this->assertSame(2, count($feed->entries[0]->contributors));
    }
}
