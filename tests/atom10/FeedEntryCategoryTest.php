...
class Atom10_FeedEntryCategoryTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $input = sprintf('<feed xmlns="http://www.w3.org/2005/Atom"><entry><category %s/></entry></feed>', $input);
        $feed = \ComplexPie\ComplexPie($input);
        list($category) = $feed->entries[0]->categories;
        if ($expected !== null)
        {
    ...
    {
        $input = '<feed xmlns="http://www.w3.org/2005/Atom"><entry><category label="FAIL"/></entry></feed>';
        $feed = \ComplexPie\ComplexPie($input);
        list($category) = $feed->entries[0]->categories;
...
EOF;
        $feed = \ComplexPie\ComplexPie($input);
        $categories = $feed->entries[0]->categories;
...
EOF;
        ...
        $this->assertSame('category 1', $feed->entries[0]->categories[0]->term->to_text());
        $this->assertSame('category 2', $feed->entries[0]->categories[1]->term->to_text());
    }
...
EOF;
        $feed = \ComplexPie\ComplexPie($input);
        $this->assertSame(2, count($feed->entries[0]->categories));
    }
}
